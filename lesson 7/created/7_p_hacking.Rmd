...
dat |>
  ...
  summarize(mean_outcome1 = mean(outcome1),
            mean_outcome2 = mean(outcome2),
            mean_outcome3 = mean(outcome3),
            correlation_outcome1_outcome2 = cor(outcome1, outcome2, use = "complete.obs"),
            correlation_outcome1_outcome3 = cor(outcome1, outcome3, use = "complete.obs"),
            correlation_outcome2_outcome3 = cor(outcome2, outcome3, use = "complete.obs")
            ) |>
  mutate_if(is.numeric, round_half_up, digits = 2)
...
simulation <- 
  ...
  mutate(results_outcome1 = pmap(.l = list(data = generated_data,
         ...
         results_outcome2 = pmap(.l = list(data = generated_data,
                                           outcome = "outcome2"),
                                 .f = analyze),
         results_outcome3 = pmap(.l = list(data = generated_data,
                                           outcome = "outcome3"),
                                 .f = analyze))
...
simulation_summary <- simulation |>
  ...
  rename(outcome3_p = p) |>
  # simulate flexible reporting
  mutate(hacked_p = case_when(outcome1_p < .05 ~ outcome1_p,
                              outcome2_p < .05 ~ outcome2_p,
                              outcome3_p < .05 ~ outcome3_p,
                              TRUE ~ outcome1_p)
         )|>
  # summarize
  group_by(n_per_condition,
  ...
  summarize(prop_sig_outcome1 = mean(outcome1_p < .05),
            prop_sig_outcome2 = mean(outcome2_p < .05),
            prop_sig_outcome3 = mean(outcome3_p < .05),
            prop_sig_hacked = mean(hacked_p < .05),
            .groups = "drop")
```
...
The proportion of significant results for each outcome variable alone is negligable the same. However, the hacked proportion of significant results is now much higher in case for the three outcome variables than was the case for the two outcome variables. This is true across all correlations between the outcomes.

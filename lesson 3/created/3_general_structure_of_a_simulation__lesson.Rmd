...
#calculate a t.test on simluated data
...
data_generation <- function(n_control, n_intervention, mean_control, mean_intervention, sd_control, sd_intervention) {
...
data_generation(n_control, n_intervention, mean_control, mean_intervention, sd_control, sd_intervention)
...
generate_data <- function(n_control, # the parameters are now function arguments
                          ...
                          mean_control,
                          mean_intervention,
                          sd_control = mean_control,
                          sd_intervention = mean_intervention) {
...
t_test <- function(x) {
  ...
  res_ttest <- t.test(formula = score ~ condition, 
       data = x, 
       var.equal = TRUE,
       alternative = "two.sided")
  
  res <- tibble(p = res_ttest$p.value)

  return(res)
}

t_test(x = generated_data)
```
...
t_test_p_value(x = generated_data)

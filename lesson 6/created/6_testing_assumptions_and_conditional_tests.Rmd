...
analyze_data_wilcox <- function(data) {
  ...
  hypothesis_test_wilcox <- wilcox.test(formula = score ~ condition,
                             data = data,
                             alternative = "two.sided")
  
  res <- tibble(hypothesis_test_p_wilcox = hypothesis_test_wilcox$p.value)
...
test_assumption_of_normality <- function(data) {
  ...
  res <- tibble(assumption_test_p_shapirowilks_intervention = fit_intervention$p.value,
                assumotion_test_p_shapirowilks_control = fit_control$p.value)
...
- What interpretation issues might be arguments against doing this by default?
...
simulation_summary <- simulation |>
  ...
  summarize(proportion_significant_conditional = mean(hypothesis_p_conditional < .05),
            ...
            .groups = "drop") |>
  mutate(power_diff_t_vs_con = proportion_significant_students_t - proportion_significant_conditional,
         power_diff_t_vs_wil = proportion_significant_students_t - proportion_significant_wilcox)

# plots
ggplot(simulation_summary, aes(n*2, proportion_significant_conditional, color = skew, alpha = scale, group = interaction(mu_intervention, scale))) +
  geom_hline(yintercept = 0.80, linetype = "dotted") +
  geom_hline(yintercept = 0.05, linetype = "dotted") +
  geom_point() +
  geom_line() +
  ...
  theme_linedraw() +
  scale_color_viridis_d(begin = 0.2, end = 0.8) +
  ggtitle("Power of conditional Student's t-test vs. Wilcox test for Cohen's d  of = 0 | 0.5") +
  facet_wrap(~ mu_intervention)

ggplot(simulation_summary, aes(n*2, proportion_significant_wilcox, color = skew, alpha = scale, group = interaction(mu_intervention, scale))) +
  geom_hline(yintercept = 0.80, linetype = "dotted") +
  geom_hline(yintercept = 0.05, linetype = "dotted") +
  geom_point() +
  geom_line() +
  ...
  theme_linedraw() +
  scale_color_viridis_d(begin = 0.2, end = 0.8) +
  ggtitle("Power of Wilcox test for Cohen's d  of = 0 | 0.5") +
  facet_wrap(~ mu_intervention)

ggplot(simulation_summary, aes(n*2, proportion_significant_students_t, color = skew, alpha = scale, group = interaction(mu_intervention, scale))) +
  geom_hline(yintercept = 0.80, linetype = "dotted") +
  geom_hline(yintercept = 0.05, linetype = "dotted") +
  geom_point() +
  geom_line() +
  ...
  theme_linedraw() +
  scale_color_viridis_d(begin = 0.2, end = 0.8) +
  ggtitle("Power of students t-test for Cohen's d  of = 0 | 0.5") +
  facet_wrap(~ mu_intervention)

ggplot(simulation_summary, aes(power_diff_t_vs_con)) +
  geom_histogram(binwidth = 0.01) +
  theme_linedraw() +
  xlab("Difference in power between students t-test \nvs. conditional testing")

ggplot(simulation_summary, aes(power_diff_t_vs_wil)) +
  geom_histogram(binwidth = 0.01) +
  theme_linedraw() +
  xlab("Difference in power between students t-test \nvs. Wilcoxon rank-sum test")
...
- Use the same sample sizes as in the above simulation (i.e., `n = c(10, 25, 50, 100, 200)`).
...
assumption_test_p_levene <- function(data) {
  
  data <- data |>
    mutate(condition = as.factor(condition))
  
  assumption_test_levene <- leveneTest(score ~ condition,
                                       data = data)
  
  res <- tibble(assumption_test_p_levene = assumption_test_levene$`Pr(>F)`[1]) 
...
experiment_parameters <- expand_grid(
  ...
  mu_control = 0,
  mu_intervention = 0.5, 
  sigma_control = c(1, 1.5, 2),
  sigma_intervention = c(1, 1.5, 2),
  skew = c(0, 2, 4, 8, 12),     # slant/skew. When 0, produces normal/gaussian data
  iteration = 1:100
) |>
  # calculate skew-normal parameters
  # don't worry about the math, it's not important to understand
  mutate(delta = skew / sqrt(1 + skew^2),
         scale_control = sigma_control / sqrt(1 - 2 * delta^2 / pi),
         scale_intervention = sigma_intervention / sqrt(1 - 2 * delta^2 / pi),
         location_control = mu_control - scale_control * delta * sqrt(2 / pi),
         location_intervention = mu_intervention - scale_intervention * delta * sqrt(2 / pi)) 
...
simulation_summary <- simulation |>
  ...
  unnest(analysis_results_normality) |>
  # condition logic
  mutate(hypothesis_p_conditional = ifelse(assumption_test_p_levene < .05, 
                                           hypothesis_test_p_wilcox,
                                           hypothesis_test_p_students_t)) |>
  # summarize
  mutate(skew = as.factor(skew)) |>
  ...
  summarize(proportion_significant_conditional = mean(hypothesis_p_conditional < .05),
            proportion_significant_wilcox = mean(hypothesis_test_p_wilcox < .05),
            .groups = "drop") |>
  mutate(power_diff_con_vs_wil = proportion_significant_conditional - proportion_significant_wilcox)

#plot
ggplot(simulation_summary, aes(power_diff_con_vs_wil)) +
  geom_histogram(binwidth = 0.01) +
  theme_linedraw() +
  xlab("Difference in power between conditional tests\nvs. always running non-parametric")

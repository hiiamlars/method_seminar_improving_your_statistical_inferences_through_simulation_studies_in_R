...
You don't need to understand the following math, but here is is for nerds.
...
# 3. Impact of non-normality on the t-test's false-positive and true-positive (power) rates amd estimates of Cohens's d
...
# define experiment parameters ----
experiment_parameters <- expand_grid(
  n_control = seq(10, 90, 40), # multiple sample sizes
  mu_control = 0,
  mu_intervention = c(0, 0.5), # multiple location values
  sigma_control = 1,
  sigma_intervention = 1,
  skew_control = c(0, 24), # higher skew value
  iteration = 1:1000
) |>
  mutate(n_intervention = n_control,
         skew_intervention = skew_control) |> 
  
  # make an intuitive label for the conditions
  mutate(distribution = case_when(skew_intervention == 0 ~ "Normal data",
                                  skew_intervention == 24 ~ "High Skew-Normal data"),
         population_effect_size = paste("Cohen's d =", mu_intervention),
         n_per_condition = factor(n_control)) |>
...
# do some sanity checks on the experiment's grid
experiment_parameters |>
  count(n_per_condition, population_effect_size, distribution) |>
  kable() |>
  kable_classic(full_width = FALSE)
...
simulation_summary |>
  mutate_if(is.numeric, janitor::round_half_up, digits = 2) |>
  arrange(n_per_condition) |>
  kable() |>
  kable_classic(full_width = FALSE)

#additional parameter analysis
summary_analysis <- simulation_summary |> 
  filter(distribution == "High Skew-Normal data")
...
analyze <- function(data) {
  ...
  res_welches_t_test <- t.test(formula = score ~ condition, 
                               data = data,
                               var.equal = FALSE,
                               alternative = "two.sided")
  
  res_students_t_test <- t.test(formula = score ~ condition, 
                                data = data,
                                var.equal = TRUE,
                                alternative = "two.sided")
  
  res <- tibble(p_welches = res_welches_t_test$p.value, 
                p_students = res_students_t_test$p.value)
...
experiment_parameters <- expand_grid(
  n_control = seq(10, 90, 40), #multiple sample sizes
  n_intervention = seq(10, 90, 40), #multiple sample sizes
  mu_control = 0,
  mu_intervention = c(0, 0.5), #multiple location values
  sigma_control = 1,
  sigma_intervention = c(1, 3, 5), # multiple scale values (SDs)
  skew_control = 0, #fixed non-skewed data
  iteration = 1:1000
) |>
  mutate(skew_intervention = skew_control) |> 
  
  # make an intuitive label for the conditions
  mutate(sd_ratio = factor(sigma_intervention,
                           levels = c(1, 3, 5),
                           labels = c("sd_ratio = 1", "sd_ratio = 3", "sd_ratio = 5")),
         population_effect_size = paste("Cohen's d =", mu_intervention)
         )|>
...
# do some sanity checks on the experiment's grid
experiment_parameters |>
  count(n_control, n_intervention, sd_ratio, population_effect_size) |>
  kable() |>
  kable_classic(full_width = FALSE)
...
simulation_summary <- simulation |>
  ...
  group_by(n_control,
           ...
           sd_ratio,
           population_effect_size) |>
  summarize(proportion_students = mean(p_students < .05),
            proportion_welches = mean(p_welches < .05)) |> 
  select(n_control,
         n_intervention,
         sd_ratio,
         population_effect_size,
...
# filter for most relevant condition
filtered_alpha_summary <- simulation_summary |> 
  filter(n_control != n_intervention & sd_ratio != "sd_ratio = 1")

# table
filtered_alpha_summary |>
  mutate_if(is.numeric, janitor::round_half_up, digits = 2) |>
  arrange(sd_ratio) |>
  kable() |>
  kable_classic(full_width = FALSE)

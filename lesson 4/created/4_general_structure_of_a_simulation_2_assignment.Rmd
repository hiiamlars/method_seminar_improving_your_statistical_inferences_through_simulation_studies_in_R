...
# simulation parameters
experiment_parameters <- expand_grid(
  n_per_condition = c(25, 50, 75, 100),
  mean_control = 0,
...
generate_data <- function(n_per_condition,
  ...
  data_control <- 
    tibble(condition = "control", levels = c("intervention", "control"),
           score = rnorm(n = n_per_condition, mean = mean_control, sd = sd))
  
  data_intervention <- 
    tibble(condition = "intervention", levels = c("intervention", "control"),
           score = rnorm(n = n_per_condition, mean = mean_intervention, sd = sd))
...
analyze <- function(data) {
  ...
  res2 <- tibble(cohens_d = res_cohens_d$estimate)
    
  return(data.frame(p = res, cohens_d = res2))
}
...
simulation_summary <- simulation |>
  ...
  # for each level of mean_intervention... 
  group_by(population_effect_size) |>
    # ... calculate proportion of iterations where significant results were found and the average Cohen's d and p-value
  mutate(significant = p < .05) |>
  summarize(proportion_of_significant_p_values = mean(significant), average_cohens_d = mean(cohens_d))
